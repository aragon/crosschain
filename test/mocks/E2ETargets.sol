...
contract GuardedTarget {
    ...
    ///         chain this must be the EXECUTOR, never the controller or the
    ///         adapter.
...
contract ShortReturnAdapterMock {
    ...
    ///         single word instead of `(uint256, uint256)`.
    function sendMessage(address, uint256, uint256, bytes calldata) external payable returns (bytes32) {
        return bytes32(uint256(1));
    }
}

/// @notice An action target that calls `execute` straight back on whoever
///         invoked it.
/// @dev DO NOT USE IN PRODUCTION! Test-only.
///      Run as an action, its caller is the execution target itself, so this
///      re-enters `execute` while the outer `execute` is still on the stack --
///      which the `nonReentrant` guard must refuse.
contract CallerReenterer {
    function callBackCaller() external {
        IExecutor(msg.sender).execute(keccak256("reentrant"), new Action[](0), 0);
    }
}
