...
import { RejectingReceiver, ShortReturnAdapterMock } from "@mocks/E2ETargets.sol";

/// @title CrossChainFeesAndOpsE2ETest
/// @notice Who pays, what happens when they cannot, and how funds get back out
///         -- the paths an operator runs into rather than a user.
contract CrossChainFeesAndOpsE2ETest is CrossChainE2EBase {
    ...
    /// @dev Deploys the ERC20-fee lane lazily: only the fee-token tests need it,
    ///      and it doubles the setup cost of every other test in the file.
    function _useErc20Lane() internal {
        (erc20Origin, erc20Destination) = _deployLane(
            ORIGIN_CHAIN_ID, ORIGIN_SELECTOR, DESTINATION_CHAIN_ID, DESTINATION_SELECTOR, address(feeToken)
        );
    ...
    /// @notice The ERC20 fee is pulled from the CONTROLLER by the Router, and no
    ///         standing allowance is left behind.
    /// @dev The allowance assertion is the regression test for the reset that
    ///      follows the approval: an adapter that approved but never zeroed
    ///      would leave the controller's treasury permanently drainable by the
    ///      Router.
    function test_e2e_erc20FeeIsPulledFromTheControllerAndAllowanceIsReset() public {
        _useErc20Lane();

        uint256 controllerBefore = feeToken.balanceOf(address(erc20Origin.controller));

        _forwardViaProposal(erc20Origin, erc20Destination, GAS_LIMIT, _cancelPayload(erc20Destination));

        assertEq(
            feeToken.balanceOf(address(erc20Origin.controller)),
            controllerBefore - FEE,
            "the controller must have paid the ERC20 fee"
        );
        assertEq(feeToken.balanceOf(address(erc20Origin.router)), FEE, "the router must have pulled the fee");
        assertEq(
            feeToken.allowance(address(erc20Origin.controller), address(erc20Origin.router)),
            0,
            "no standing allowance may be left on the controller"
        );
        assertEq(feeToken.balanceOf(address(erc20Origin.adapter)), 0, "no funds may ever sit on the adapter");
    }

    /// @notice An ERC20-fee lane still carries the message all the way.
    /// @dev The fee token is an origin-side concern only; nothing about it
    ///      should reach the destination.
    function test_e2e_erc20LaneDeliversAndExecutes() public {
        _useErc20Lane();

        bytes32 txId = _forwardViaProposal(erc20Origin, erc20Destination, GAS_LIMIT, _cancelPayload(erc20Destination));

        (, bool success) = _deliverNext(erc20Origin, erc20Destination);

        assertTrue(success);
        _assertExecuted(erc20Destination, txId);
        assertEq(erc20Destination.target.cancellations(), 1);
    }
    ...
    function test_e2e_erc20QuoteReportsTheTokenAndTheBalance() public {
        ...
        (address token, uint256 fee, uint256 available) =
            erc20Origin.controller.quoteFee(erc20Destination.chainId, GAS_LIMIT, _cancelPayload(erc20Destination));
    ...
    /// @notice An underfunded controller fails with the dedicated ops error, and
    ///         the same send works once it is topped up.
    function test_e2e_insufficientNativeBalanceRevertsThenSucceedsAfterTopUp() public {
        // Strand the controller just below the fee.
        vm.deal(address(origin.controller), FEE - 1);

        vm.prank(address(origin.dao));
        vm.expectRevert(abi.encodeWithSelector(Errors.INSUFFICIENT_FEE_BALANCE.selector, address(0), FEE, FEE - 1));
    ...
    function test_e2e_insufficientErc20BalanceRevertsWithTheToken() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Errors.INSUFFICIENT_FEE_BALANCE.selector, address(feeToken), FEE, FEE - 1)
        );
        erc20Origin.controller.forwardMessage(erc20Destination.chainId, GAS_LIMIT, _cancelPayload(erc20Destination));
    }
    ...
    ///         ADAPTER, before the send is ever attempted.
    /// @dev The adapter asks `isChainSupported` first and raises its own
    ///      `DESTINATION_CHAIN_ID_NOT_SUPPORTED` rather than letting the Router
    ///      revert with `UnsupportedDestinationChain`. The distinction matters
    ///      operationally: the failure names the lane in OUR vocabulary, and no
    ...
    function test_e2e_unsupportedLaneIsRejectedByTheAdapterBeforeTheRouter() public {
        // A chain the ADAPTER can map but the ROUTER has no peer for. The
        // registry entry is what makes it mappable.
        ...
        vm.prank(address(origin.dao));
        vm.expectRevert(
            abi.encodeWithSelector(Errors.DESTINATION_CHAIN_ID_NOT_SUPPORTED.selector, uint256(THIRD_SELECTOR))
        );
        origin.controller.forwardMessage(THIRD_CHAIN_ID, GAS_LIMIT, _emptyPayload());

        assertEq(origin.router.sentCount(), 0, "the router must never have been reached");
        assertEq(address(origin.controller).balance, balanceBefore, "no fee may be charged for a rejected lane");
    }

    /// @notice A lane whose local adapter has NO CODE cannot even be
    ///         configured, so the silent-success failure mode is unreachable.
    /// @dev The EVM reports success for a `delegatecall` into a codeless
    ///      address, which would let a proposal "execute" while nothing was ever
    ///      bridged. `updateConfig` closes that off at the door rather than
    ///      relying on the return-size check downstream.
    function test_e2e_codelessLocalAdapterCannotBeConfigured() public {
        ...
        configs[0] =
            ICrossChainController.ChainConfig({ localAdapter: codeless, remoteAdapter: address(destination.adapter) });

        vm.prank(address(origin.dao));
        vm.expectRevert(abi.encodeWithSelector(Errors.HAS_NO_CODE.selector, codeless));
    ...
    /// @dev The EVM reports success for a `delegatecall` that returns too
    ///      little. Without the return-size check a proposal would "execute"
    ...
    function test_e2e_shortAdapterReturnDataIsRejected() public {
        ...
        _configureLaneWithLocalAdapter(origin, destination.chainId, address(badAdapter), address(destination.adapter));
    ...
    ///      not through the send. This is why `Errors.UNEXPECTED_NATIVE_VALUE`
    ///      is unreachable from the controller.
    function test_e2e_forwardMessageRejectsAttachedValue() public {
        vm.deal(address(origin.dao), 1 ether);

        bytes memory call =
            abi.encodeCall(ICrossChainController.forwardMessage, (destination.chainId, GAS_LIMIT, _emptyPayload()));

        vm.prank(address(origin.dao));
        // solhint-disable-next-line avoid-low-level-calls
        (bool withValue,) = address(origin.controller).call{ value: 1 ether }(call);
    ...
    function test_e2e_sweepReturnsErc20FundingToTheDao() public {
        ...
        vm.prank(address(origin.dao));
        origin.controller.sweep(address(feeToken), address(origin.dao), 3 ether);
    ...
    function test_e2e_sweepToARejectingReceiverReverts() public {
        ...
        vm.prank(address(origin.dao));
        vm.expectRevert(abi.encodeWithSelector(Errors.NATIVE_TRANSFER_FAILED.selector, address(rejector), 1 ether));
        origin.controller.sweep(address(0), address(rejector), 1 ether);
    }

    /// @notice Sweeping the whole balance strands the next send. The ops-runbook
    ///         consequence of the controller being the fee payer.
    function test_e2e_sweepingEverythingStrandsTheNextSend() public {
        vm.prank(address(origin.dao));
        origin.controller.sweep(address(0), address(origin.dao), address(origin.controller).balance);
    ...
    /// @notice Sweeping is still available while the controller is paused.
    /// @dev Deliberate: an incident that freezes the message paths must not also
    ///      trap the treasury's pre-funding inside the controller.
    function test_e2e_sweepStillWorksWhilePaused() public {
        vm.prank(guardian);
        origin.controller.pause();

        uint256 daoBefore = address(origin.dao).balance;

        vm.prank(address(origin.dao));
        origin.controller.sweep(address(0), address(origin.dao), 1 ether);

        assertEq(address(origin.dao).balance, daoBefore + 1 ether);
    }
}
