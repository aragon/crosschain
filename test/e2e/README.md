...
topology configs are transcribed from - seeded into a real `ChainIdRegistry` and
read back through the adapter bound to it. A wrong entry there is only
discoverable against a live Router, and the deploy kit cannot catch it: a
non-zero selector naming the wrong chain resolves fine and deploys silently.
