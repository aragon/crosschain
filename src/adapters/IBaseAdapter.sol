...
/// @dev The two paths run in different execution contexts: `sendMessage` is
///      reached only by `delegatecall` from the controller, while the receive
///      path runs as the adapter itself so it can read its own storage.
